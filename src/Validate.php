...
use Think\Component\Validate\ValidateRule;
...
class Validate
{
    ...
    {
        if (!empty($lang)) {
            $file = __DIR__ . DIRECTORY_SEPARATOR . 'Lang' . DIRECTORY_SEPARATOR . $this->lang . '.php';
            if (is_file($file)) {
                $typeMsg = require $file;
        ...
        foreach ($rules as $key => $rule) {
            ...
            if (false === $result) {
                ...
                if (!empty($msg[$i])) {
                    ...
                    $message = $this->getRuleMsg($field, $title, $value, $info, $rule);
                }
     ...
     * @return string|array
     */
    protected function getRuleMsg(string $attribute, string $title, $value, string $type, $rule)
    {
    ...
    protected function parseErrorMsg(string $msg, $rule, string $title, $value)
    {
        ...
        if (is_scalar($rule) && false !== strpos($msg, ':')) {
            ...
            $msg = str_replace(
                ...
                [$title, $value, $array[0], $array[1], $array[2]],
                $msg
            );
