...
namespace Think\Component\Validate;
...
use Think\Component\Validate\Contract\Enumable;
use Think\Component\Validate\Exception\ValidateException;
use Think\Component\Validate\Validate\ValidateRule;
...
class Validate
{
    ...
    protected $typeMsg = [
        ...
        'notConformRules' => 'not conform to the rules',
    ];
        ...
        foreach ($items as $k => $item) {
            ...
            foreach ($values as $value) {
                ...
                if (true !== $result) {
                    // 验证失败 记录错误信息
                    if (false === $result) {
                        $result = $this->getRuleMsg($name, $title, '', $item, $value);
                    }
        ...
        foreach ($rules as $key => $rule) {
            ...
            if (false === $result) {
                ...
                if (!empty($msg[$i])) {
                    ...
                    $message = $this->getRuleMsg($field, $title, $type, $rule, $value);
                }

                return $message;
            } elseif (true !== $result) {
                // 返回自定义错误信息
                return $this->parseUserErrorMessage($result, $title, $rule, $value);
            }
            $i++;
        }
    ...
    protected function parseUserErrorMessage($message, $title, $rule, $value)
    {
     ...
     * @return string|array
     */
    protected function getRuleMsg(string $attribute, string $title, string $type, $rule, $value)
    {
        if (isset($this->message[$attribute . '.' . $type])) {
            ...
            $msg = $title . $this->typeMsg['notConformRules'];
        }
        ...
        return $this->parseErrorMsg($msg, $rule, $title, $value);
    }
    ...
    protected function parseErrorMsg(string $msg, $rule, string $title, $value)
    {
        ...
        if (is_scalar($rule) && str_contains($msg, ':')) {
            ...
            if (str_contains($msg, ':value')) {
                $msg = str_replace(':value', (string) $value, $msg);
            }
        }
