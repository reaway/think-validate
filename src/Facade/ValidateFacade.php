...
/**
 * @see Validate
 * @package Think\Component\Facade\Facade
 * @mixin Validate
 * @method static Validate rule(string|array $name, mixed $rule = '') 添加字段验证规则
 * @method static Validate extend(string $type, callable $callback = null, string $message = null) 注册验证（类型）规则
 ...
 * @method static Validate scene(string $name) 设置验证场景
 * @method static bool hasScene(string $name) 判断是否存在某个验证场景
 * @method static Validate batch(bool $batch = true) 设置批量验证
 * @method static Validate failException(bool $fail = true) 设置验证失败后是否抛出异常
 * @method static Validate only(array $fields) 指定需要验证的字段列表
 * @method static Validate remove(string|array $field, mixed $rule = null) 移除某个字段的验证规则
 * @method static Validate append(string|array $field, mixed $rule = null) 追加某个字段的验证规则
